#{character or int: ((function,arg),(function,arg),..)}
...
#I wonder if I need to write my own keyevent code to handle the switch between key up and key down
...
class Interface:
	...
	def onKeyEvent(self,key):
		if(key.vk==ltc.KEY_CHAR):
			if self.keyBinding.has_key(chr(key.c)):
				return self.call(self.keyBinding[chr(key.c)])
		else:
			if self.keyBinding.has_key(key.vk):
				return Interface.call(self.keyBinding[key.vk])
